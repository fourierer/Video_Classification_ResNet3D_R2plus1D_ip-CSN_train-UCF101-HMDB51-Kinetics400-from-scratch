...
def video_process(video_file_path, dst_root_path, ext, fps=-1, size=240):
    ...
    frame_rate = frame_rate[0] / frame_rate[1] # 获取帧率
    duration = float(res[3]) # 获取视频时长
    n_frames = int(frame_rate * duration) # 获取总帧数

    name = video_file_path.stem # 获取视频名称
    dst_dir_path = dst_root_path / name
    dst_dir_path.mkdir(exist_ok=True) # 在图像文件夹中用视频名称新建文件夹
    n_exist_frames = len([
    ...
    #print(ffmpeg_cmd)
    subprocess.run(ffmpeg_cmd) # 抽帧命令行
    #print('\n')
